...
/**
 * Monta um calend�rio de eventos
...
CalendarioEventos.prototype = {
	exibir : function( divname )
	{
		...
		if( !el )
		{
			alert('DIV n�o encontrada: ' + divname);
			return false;
		}
	...
	_montar : function()
	{
		...
		if(this.mes == 1 && this.ano % 4 == 0)  // ano bissexto e � fevereiro
		{
			totalDias ++;
		}
		...
		// monta os dias vazios do m�s (inicio do mes)
...
$list = $this->obj->_getObjectPart('_foreign');

foreach($list as $name => $item){
	switch($item['type']){
		case Lumine_Base::MANY_TO_MANY:
			?>
			<tr>
			  <td bgcolor="#FFFFFF" valign="top" align="right"><?php echo ucfirst($name); ?></td>
			  <td bgcolor="#FFFFFF">
			<?php 
			  ...
			  $obj = new $item['class'];
			  $obj->find();
			  $pk = $obj->_getPrimaryKeys();
			  
				while($obj->fetch()){
					printf('<input type="checkbox" name="%s[]" value="%s" %s /> %s<br />',
						$name,
						$obj->$pk[0]['name'],
						!empty($_POST[$name]) && in_array($obj->$pk[0]['name'], $_POST[$name]) ? ' checked="checked"' : '',
						$obj->nome == '' ? $obj->$pk[0]['name'] : $obj->nome
					);
				}
			  ?>
			  </td>

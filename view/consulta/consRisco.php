<?
	...
	//desregistra caso nao seja update
	unset($_SESSION["upd_risco"]);
	...
	$riscos = $riscoController->listAll();
...
<div id="conteudo" class="column">
			...
			<h3><? echo $nome_projeto_logado; ?> / Riscos</h3>
				<fieldset style="width:850px;">
					<legend></legend>
						<form action="<? $PHP_SELF; ?>" method="post">
							<p><label for="ordem">Ordenar por:</label>
							<select size="1" name="ordem" style="width:180px;">
								<option value="0" <? if ($_SESSION["ordem_log"] == '0') echo "selected"; ?>>Tipo</option>
								<option value="1" <? if ($_SESSION["ordem_log"] == '1') echo "selected"; ?>>Probabilidade</option>
								<option value="2" <? if ($_SESSION["ordem_log"] == '2') echo "selected"; ?>>Efeito</option>
							</select><input name="BTSearch" class="formbutton" value="Aplicar" type="submit" style="margin-left:20px;"/></p>
							
							<table cellpading="0" cellspacing="5" border="0">
								<tbody>
								<?=$riscoController->printConsulta($riscos)?>
							</table>
								<br>
